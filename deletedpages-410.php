<?php
/*
Plugin Name: 410 Delete Pages SEO
Plugin URI: #
Description: Force 410 HTTP Status on listed pages, or just articles/pages that no longer exist.
Version: 0.0.5
Author: Marc Moeller
Author URI: http://moellerseo.com/
License: GPL3
*/
...
class deletepages_410 {
	...
	private function max_404_list_length() {
		return get_option( 'deletepages_410_max_404s', 50 );
	}
	...
	function upgrade_check() {
		$options_version = get_option( 'deletepages_410_options_version', 0 );
		...
		if( $options_version < 3 ) {
			$old_links = get_option( 'deletepages_410_links_list', array() );
			...
			delete_option( 'deletepages_410_links_list' );	// remove old option
		}

		update_option( 'deletepages_410_options_version', self::db_version );
	}

	function settings_menu() {
		add_submenu_page('plugins.php', '410 Deleted Pages', '410 Deleted Pages', 'manage_options', 'deletepages_410_settings', array( $this, 'settings_page') );
	}
	...
	function settings_page() {
		...
		if( isset( $_POST['delete-from-410-list'] ) && !empty( $_POST['old_links_to_remove'] ) ) {
			check_admin_referer( 'deletepages-410-settings' );
		...
		else if ( isset( $_POST['toggle_all_in_list'] ) ) {
			if($_POST['deletepages_410_all_in_list']==1){update_option( 'deletepages_410_all_in_list', 1 );}else update_option( 'deletepages_410_all_in_list', 0 );		
		}
		
		update_option( 'deletepages_410_max_404s', 0 );
	...
	.testurl {float:right;margin-right:20px;}
	...
	<div class="wrap">
	<h2>410 Deleted WordPress Pages</h2>
	<?php if( WP_CACHE ) :?>
		<div class="updated">
	...
	<?php endif; ?>
	<p>This plugin will issue a HTTP 410 response to all pages on the list or just the pages that returns 404 page not found. This informs robots that the requested page has been permanently removed, and that they should stop trying to access it.</p>
	...
	<h3>URLs Set To 410 Response</h3>
	...
	if( empty( $links ) ) {
		...
		echo '<br><div class="searchbar"><p><b>'.count($links).'</b> records found</p><p style="display: flex; align-items: center;"><input type="text" name="searchrecord" id="searchrecord" placeholder="Search" /><input type="button" id="go" name="go" value="GO" /></p></div>';
		echo '<div class="wp-410-table-wrap"><table id="wp_gone_old_links" class="wp-list-table widefat fixed">';
		echo '<thead><th class="check-column"><input type="checkbox" id="select-all-410" /><label for="select-all-410" class="screen-reader-text"> Select all</label></th><th>Select All</th></thead>';
		echo '<tbody>';
		...
		foreach( array_keys( $links ) as $k ) {
			...
			echo "<tr $class><td><input type='checkbox' name='old_links_to_remove[]' id='wp-410-$k_attr' value='$k_attr' /></td><td><label for='wp-410-$k_attr'><code>$k</code> </label><label class='testurl'><a href='$k_attr' target='_blank'>Test URL</a></label></td</tr>";
		}
		...
		wp_nonce_field( 'deletepages-410-settings' );
	...
	<p>
	<label>
	<?php echo '<input type="checkbox" name="deletepages_410_all_in_list" value="1" ' . checked( 1, get_option( 'deletepages_410_all_in_list' ), false ) . ' />'; ?>
	Enable 410 redicts even if URL is not a 404
	</label>
	</p>
	<p class="submit" style="margin-top:10px;"><input class="button button-primary" type="submit" name="toggle_all_in_list" value="Save" /></p>
	</form>
	<hr>	
	<form action="" method="post">
	<?php
	...
	<?php wp_nonce_field( 'deletepages-410-settings' ); ?>
...
<?php
	...
	function check_for_410() {
		if(get_option( 'deletepages_410_all_in_list' )==0){
			if( !is_404() )
			return;
		}
		...
		foreach( $links as $link ) {
			if( @preg_match( $link->gone_regex, $req ) ) {
				...
				status_header( 410 );
				do_action( 'deletepages_410_response' );	// you can use this to customise the response message
